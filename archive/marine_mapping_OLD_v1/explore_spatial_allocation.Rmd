...
Read in and combine all farm data 
```{r}
...
Read in and combine all FAO tonnage data per country
```{r}
bivalves_tonnes <- read_csv(here::here("marine/bivalve/bivalve_farms/data/fao_mariculture_bivalve.csv")) %>%
  mutate(group = "bivalve")
  
shrimp_tonnes <- read_csv(here::here("marine/shrimp/shrimp_farms/data/fao_mariculture_shrimp.csv")) %>%
  mutate(group = "shrimp") %>%
  filter(year == 2017)
  
marine_fish_tonnes <- read_csv(here::here("marine/marine_fish_general/finfish_farms/data/fao_mariculture_finfish.csv")) %>%
  mutate(group = "marine_fish_general") %>%
  filter(year == 2017)
  
salmon_tonnes <- read_csv(here::here("marine/salmon/salmon_farm/data/fao_mariculture_salmon.csv")) %>%
  mutate(group = "salmon") %>%
  filter(year == 2017)
...
all_species_country_tonnes <- rbind(bivalves_tonnes, shrimp_tonnes, marine_fish_tonnes, salmon_tonnes, crustaceans_tonnes, tuna_tonnes)
```
...
## make a dataset with the country, species type (farm type), tonnage, and # of farms, for those that score data quality %in% c(3, 4, 5)... review all of these, since many of these are just subsets of total number of farms...

sum(all_species_country_tonnes$fao_tonnes_production) # 29623515

n_farms_final <- n_farms %>%
  filter(data_quality %in% c(3,4,5))

n_farms_final <- n_farms %>%
  filter(data_quality %in% c(3,4,5)) %>%
  group_by(iso3c, group) %>%
  summarise(n_farms = sum(n_farms)) %>%
  left_join(all_species_country_tonnes, by = c("iso3c", "group")) %>%
  dplyr::select(iso3c, country, year, group, n_farms, fao_tonnes_production)
sum(n_farms_final$fao_tonnes_production) # 26812832

n_farms_tonnes_no_info <- n_farms %>%
  filter(data_quality %in% c(1, 2)) %>%
  group_by(iso3c, group) %>%
  summarise(n_farms = sum(n_farms)) %>%
  left_join(all_species_country_tonnes, by = c("iso3c", "group")) %>%
  mutate(n_farms = NA)
sum(n_farms_tonnes_no_info$fao_tonnes_production) # 2810682

26812832 + 2810682 # 29623514 # perfect

n_farms_final_final <- rbind(n_farms_final, n_farms_tonnes_no_info)
sum(n_farms_final_final$fao_tonnes_production) # 29623515


## save n_farms_final to use to make a model of farms and production
write.csv(n_farms_final_final, "data/n_farms_production_data.csv", row.names = FALSE)
...
countries <- data.frame(iso3c = unique(all_farms_sf$iso3c)) %>%
