...
taxon_list <- read.csv(here("marine/STEP1_species_groups/int/marine_groups.csv"))  %>%
...
production_sf <- st_read(file.path("/home/sgclawson/github/food_systems/aquaculture/marine/bivalve/bivalve_nutrient/prod_data_by_cell_iso3c.shp")) %>%
...
sum(production_spp_props$tonnes_bivalve) + sum(production_spp_props$tonnes_gastropod) # 17435623
...
sum(nutrient_data$total_N_P_t) #  50167.83
sum(production_spp_props$tonnes_gastropod) # 422466
...
cellStats(bivalves_raster_total, stat = "sum") # 50167.83
sum(bivalves_raster_df$N_P_tonnes) # 50167.83
sum(nutrient_data$total_N_P_t, na.rm = TRUE) # 50167.83
...
cellStats(bivalves_raster_N, stat = "sum") # 44147.69
sum(bivalves_raster_df$N_tonnes) # 44147.69
sum(nutrient_data$N_poll_t, na.rm = TRUE) # 44147.69
