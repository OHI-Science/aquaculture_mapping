...
farms_all <- ASC_bivalve %>%
  cbind(iso) %>%
  mutate(iso3c = ifelse(str_detect(comp_nm, "Jersey"), "JEY", iso3c))
...
farms_esp <- data.frame(lat = c(38.345509445679276, 36.83403538312481, 43.6586646127094, 39.578993240236755, 36.55518316813145, 43.488143305644364, 39.98227981322492, 41.893397069777485, 36.69547948740147, 43.36941893593161, 28.17841973419213, 36.705816371679525, 42.42511968211785, 28.464013540472575, 41.109958666992036, 39.47564987283762, 43.436972390962005),
                        ...
                        subregion = c("Alicante", "Almeria", "Asturias", "Baleares", "Cadiz", "Cantabria", "Castellon", "Gerona", "Granada", "La Coruna", "Las Palmas", "Malaga", "Pontevedra", "Santa Cruz de Tenerife", "Tarragona", "Valencia", "Vizcaya")) %>%
  mutate(iso3c = "ESP") %>%
...
## Jersey 
 ...
 - 5 licensed farms (only including "mollusc farm open"): Locations estimated from maps and addresses listed 
 - Jersey Oyster Company Limited: 49.172680993306905, -2.0255193115109957
 - Seymour Oyster Company Ltd: 49.220095937386375, -2.0215887506551438
 - La Rocque Fisheries Limited: 49.169912669200976, -2.0230904378277916
 - Jersey Sea Farms: 49.242435452408536, -2.060618920921559
 - Island Dived Scallops: 49.20656011166959, -2.0158403231425996
...
Fish and shellfish farming, 2020. . Government of Jersey: Information and public services for the Island of Jersey. [Accessed December 4, 2020]  https://www.gov.je/Industry/FarmingFishing/Fishing/Pages/FishFarming.aspx
 ...
 - UPDATE: This is not necessary. These companies are represented in the ASC data, but listed under the UK. I will fix this, so that we aren't double counting these farms. 
 
```{r, evalute = F}

# farms_jey <- data.frame(lat = c(49.172680993306905, 49.220095937386375, 49.169912669200976, 49.242435452408536, 49.20656011166959),
#                         lon = c(-2.0255193115109957, -2.0215887506551438, -2.0230904378277916, -2.060618920921559, -2.0158403231425996)) %>%
#   mutate(iso3c = "JEY") %>%
...
buff <- st_buffer(g_meters, dist = 1000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
plot(buff)
...
buff <- st_buffer(s_meters, dist = 1000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
plot(buff)
...
buff <- st_buffer(c_meters, dist = 40000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
plot(buff)
...
buff <- st_buffer(k_meters, dist = 7000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
...
buff <- st_buffer(nk_meters, dist = 7000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
plot(buff)
...
sta_catarina_world_crop <- st_crop(coast_raw, sta_catarina_world_crs) %>% ## crop to santa catarina extent
  mutate(new_id = 1) %>%
  group_by(new_id) %>%
  summarise(geometry = st_union(geometry)) %>% ## union all the geometries
  ungroup()
...
buff <- st_buffer(sc_meters, dist = 7000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
...
sta_catarina_points_lines <- sample.line(sta_catarina_crop, n = 24) ## equally place the number of farms along the coastline 


sta_catarina_points <- st_as_sf(sta_catarina_points_lines) %>%
  mutate(new_id = 1:168)  %>%
  filter(new_id %in% c(1:24, 158, 64, 78))
...
farms_bra_final <- farms_bra %>%
  ...
  mutate(data_quality = 3) %>%
  mutate(action_needed = "Allocate spatially") %>%
  mutate(subregion = "Santa Catarina")
...
# start with Capiz by placing 300 points along the coast 
...
buff <- st_buffer(cp_meters, dist = 7000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
...
# now lets place 10 points along the coast of Negros Occidental 
...
buff <- st_buffer(n_meters, dist = 5000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
...
# now lets place 250 points along the coast of samar 
...
buff <- st_buffer(sm_meters, dist = 3000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
...
samar_points_final <- samar_points %>%
  mutate(subregion = "Samar")

negros_points_final <- negros_points %>%
  mutate(subregion = "Negros Occidental")

capiz_points_final <- capiz_points %>%
  mutate(subregion = "Capiz")

## bind all three states together
all_points <- rbind(samar_points_final, negros_points_final, capiz_points_final)
mapview(all_points)
...
phl_pts_rgn <-  st_transform(phl_pts, crs = "+init=epsg:4326") %>%
  dplyr::select(subregion)

farms_phl <- data.frame(do.call(rbind, st_geometry(phl_pts_latlon)))
names(farms_phl) <- c("lon", "lat")

farms_phl_rgn <- cbind(farms_phl, phl_pts_rgn)

farms_phl_final <- farms_phl_rgn %>%
  mutate(iso3c = "PHL") %>%
  mutate(data_quality = 3) %>%
  mutate(action_needed = "Allocate spatially") %>%
  dplyr::select(-geometry)
