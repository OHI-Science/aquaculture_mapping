...
Current data is from ASC and https://zenodo.org/record/3881612, only 11 + 4737 = 4748 farms, so probably not comprehensive, just a drop in the bucket compared to the production of China. There are likely many more farms than this. Because we can't find any information on more specific locations or specific number of farms, we will gapfill later based on global tonnes/farm. For now we need to mark the data quality as 2. 

Fu, Y., Deng, J., Wang, H., Comber, A., Yang, W., Wu, W., You, S., Lin, Y., Wang, K., 2021. A new satellite-derived dataset for marine aquaculture areas in China’s coastal region. Earth Syst. Sci. Data 13, 1829–1842. https://doi.org/10.5194/essd-13-1829-2021
...
farms_chn_2 <- read.csv("/home/shares/food-systems/Food_footprint/_raw_data/Aquaculture_locations/China/split_spp/farms_biv_chn.csv") %>%
  dplyr::select(-id) %>%
  mutate(data_quality = 2)

farms_chn_fin <- rbind(farms_chn, farms_chn_2)

write_csv(farms_chn_fin, here("marine/bivalve/bivalve_farms/data/known_farm_sites_2_4_5/farms_chn.csv"))
...
We currently know the exact location of 1 ASC bivalve farm, and 23 oyster farms in Australia, however there is much more production that just 24 farms. Aquaculture websites say there are 20 abalone farms, 23 oyster farms, and 25 mussel farms in Australia 

Need to add 45 points to existing 24 points (for a total of 69 points)

 - Abalone
  - https://www.oceanwatch.org.au/aquaculture-species/
 ...
 - Oysters
  ...
  - We will use this as truth: https://oysterssa.com.au/oyster-growers/
  - We will need to place an additional 23 oyster farms in Australia 
...
farms_AUS <- read.csv(here("marine/bivalve/bivalve_farms/data/ASC_bivalve_farm_lat_lon.csv")) %>%
  ...
  mutate(data_quality = 4) ## this is an abalone farm

## now we need to add the oyster farms from this map: https://oysterssa.com.au/oyster-growers/
farms_AUS_2 <- data.frame(lat_lon = c("-32.377148325671286, 133.93965198614558", "-34.91844207220454, 137.7783987555795", "-35.77943962962783, 137.77191642975748", "-33.693162657803185, 136.924417900667", "-32.37607854727286, 133.94039259834852", "-32.373416358293625, 133.9357013204693", "-32.80283080755049, 134.1850786971314", "-34.62455022926624, 135.46366166621291", "-33.67979293746246, 136.91678920994693", "-34.551319592404916, 135.3981400818773", "-34.61868048533167, 135.4628853832039", "-32.3774179110106, 133.94079648728368", "-33.71698462803702, 136.90259542838538", "-34.90762372887531, 137.7836548109512", "-34.623471540228394, 135.46192328133426", "-33.67594103960825, 136.92673532055025", "-32.37775541730595, 133.94005795577584", "-34.52175570620221, 135.38834856713308", "-33.677446237574934, 136.92623952576452", "-32.37683026324807, 133.94078430357064", "-34.625125897259146, 135.46296226954271", "-34.61986531053386, 135.46279518888247", "-32.3779085349619, 133.9409211712551")) %>%
  separate(col = lat_lon, sep = ", ", into = c("lat", "lon")) %>% 
  mutate(iso3c = "AUS") %>%
  mutate(data_quality = 4)

all_farms_AUS <- rbind(farms_AUS, farms_AUS_2)


write.csv(all_farms_AUS, here("marine/bivalve/bivalve_farms/data/known_farm_sites_2_4_5/farms_AUS.csv"), row.names = FALSE)


all_farms_sf <- st_as_sf(all_farms_AUS, coords = c("lon", "lat"),
                         crs = crs(gadm_latlon), agr = "constant")

mapview(all_farms_sf)

## Now we will make a file that tells us the number of additional farms that need to be added to be comprehensive
AUS_additional <- data.frame(iso3c = "AUS", type = "bivalve", num_additional_farms = 45, data_quality = 4, total_farms = 69)
...
# South Korea 
We currently know the exact location of 27 bivalve farms in South Korea. However, FAO says there are 5245 shell fish leases in South Korea. WE will place an additional 5218 farms. 
...
We currently know the exact location of 25 bivalve farms in Peru. However, FAO says there are 52 bivalve farms in Peru. We will place an additional 27 farms (for a total of 52 farms). 
...
# Jersey
Current data is from ASC, only 5 farm. However, this is comprehensive. When comparing to governmental lease data, there are 5 leases in Jersey. 
...
Based on the datasets below, we have an existing 1010 bivalve farms. We need to add 193 points to existing 1010 (a final total of 1203 points)

Our current information is taken from many different state level sources, but we are definitely still missing some, as California is not included... we have farm/lease locations from 8 different states:
 ...
 - However, per FAO 2005, there were 1203 saltwater shellfish farms in the USA.. unclear if this is in marine environments, but it is the best we have. 
...
Locations of aquaculture leases in Maine, 2019. . Maine Department of Marine Resources. https://dmr-maine.opendata.arcgis.com/datasets/maine::mainedmr-aquaculture-aq-leases/about


```{r}
maine_aq_lease <- st_read("/home/shares/food-systems/Food_footprint/_raw_data/Aquaculture_locations/USA/Maine/MaineDMR_Aquaculture_AQ_Leases/MaineDMR_Aquaculture_AQ_Leases.shp")
...
maine_aq_lease_active <- maine_aq_lease %>%
  ...
  dplyr::filter(str_detect(PRIMARYSP, "Oyster|Mussel|Clam|mussels|Scallop")) %>%
  dplyr::filter(!str_detect(PRIMARYSP, "Salmon"))

unique(maine_aq_lease_active$PRIMARYSP)
  

us_me_bivalve_mol <- st_transform(maine_aq_lease_active, crs = "+init=epsg:3035") # convert to equal area projection to find centroids. 
us_me_bivalve_pts <- st_centroid(us_me_bivalve_mol)
# check
plot(st_geometry(us_me_bivalve_mol))
plot(st_geometry(st_centroid(us_me_bivalve_mol)), add=TRUE)

us_me_bivalve_pts_latlon <- st_transform(us_me_bivalve_pts, crs = "+init=epsg:4326") 

farms_us_me <- data.frame(do.call(rbind, st_geometry(us_me_bivalve_pts_latlon))) # get lat lon
names(farms_us_me) <- c("lon", "lat") 
farms_us_me <- farms_us_me %>%
  mutate(iso3c = "USA") %>%
  mutate(data_quality = 4,
         subregion = "Maine") 

mapview(us_me_bivalve_pts_latlon)
```
...
RIDEM Marine Fisheries Maps Approved Aquaculture Areas in Rhode Island, 2021. . Rhode Island Department of Environmental Management. https://ridemgis.maps.arcgis.com/apps/webappviewer/index.html?id=8beb98d758f14265a84d69758d96742f

 
```{r}
rhode_island <- esri2sf("https://risegis.ri.gov/hosting/rest/services/RIDEM/Marine_Fisheries_Map_Layers/MapServer/0")
mapview(rhode_island)

ri_active_biv <- rhode_island %>%
  filter(GPS_Source %in% c("Approved", "Expanded", "PD App")) %>%
  filter(Oysters == "Yes" | Clams == "Yes" | Scallops == "Yes")

us_ri_bivalve_mol <- st_transform(ri_active_biv, crs = "+init=epsg:3035") # convert to equal area projection to find centroids. 
us_ri_bivalve_pts <- st_centroid(us_ri_bivalve_mol)
# check
plot(st_geometry(us_ri_bivalve_mol))
plot(st_geometry(st_centroid(us_ri_bivalve_mol)), add=TRUE)

us_ri_bivalve_pts_latlon <- st_transform(us_ri_bivalve_pts, crs = "+init=epsg:4326") 

mapview(us_ri_bivalve_pts_latlon)

farms_us_ri <- data.frame(do.call(rbind, st_geometry(us_ri_bivalve_pts_latlon))) # get lat lon
names(farms_us_ri) <- c("lon", "lat") 
farms_us_ri <- farms_us_ri %>%
  mutate(iso3c = "USA") %>%
  mutate(data_quality = 4,
...
North Carolina Division of Marine Fisheries Shellfish Lease and Franchise Program, 2021. North Carolina Division of Marine Fisheries. https://deq.nc.gov/about/divisions/marine-fisheries/licenses-permits-and-leases/shellfish-lease-and-franchise#stats-and-figures

 
```{r}
url <- "https://services2.arcgis.com/kCu40SDxsCGcuUWO/arcgis/rest/services/Lease_DB_View/FeatureServer/2"
nc_shellfish <- esri2sf(url)

nc_active <- nc_shellfish %>%
  filter(Status %in% c("Active", "Approved")) # 423 active leases... wow! 

us_nc_bivalve_mol <- st_transform(nc_active, crs = "+init=epsg:3035") # convert to equal area projection to find centroids. 
us_nc_bivalve_pts <- st_centroid(us_nc_bivalve_mol)
# check
plot(st_geometry(us_nc_bivalve_mol))
plot(st_geometry(st_centroid(us_nc_bivalve_mol)), add=TRUE)

us_nc_bivalve_pts_latlon <- st_transform(us_nc_bivalve_pts, crs = "+init=epsg:4326") 

mapview(us_nc_bivalve_pts_latlon)

farms_us_nc <- data.frame(do.call(rbind, st_geometry(us_nc_bivalve_pts_latlon))) # get lat lon
names(farms_us_nc) <- c("lon", "lat") 
farms_us_nc <- farms_us_nc %>%
  mutate(iso3c = "USA") %>%
  mutate(data_quality = 4,
...
Alaska Department of Fish and Game Aquatic Farming Operations, 2020. Alaska Department of Fish and Game. https://adfg.maps.arcgis.com/apps/webappviewer/index.html?id=f3ca95493c1042b39e42a3ecb5dcad6a
...
farms_us_ak <- alaska_details %>%
  dplyr::select("lat" = "LATITUDE", "lon" = "LONGITUDE") %>%
  dplyr::mutate(iso3c = "USA", data_quality = 4, subregion = "Alaska")
...
NHDES Marine Aquaculture Data, 2020. New Hampshire Department of Environmental Services. https://granit.unh.edu/data
...
nh_aq <- st_read(file.path("/home/shares/food-systems/Food_footprint/_raw_data/Aquaculture_locations/USA/New_Hampshire/GRANIT_20211011184728/2020_Aquaculture_Sites.shp")) %>% # 40 sites, assume all active
  dplyr::filter(Shellfish == "Yes") # 39 sites

us_nh_bivalve_mol <- st_transform(nh_aq, crs = "+init=epsg:3035") # convert to equal area projection to find centroids. 
us_nh_bivalve_pts <- st_centroid(us_nh_bivalve_mol)
# check
plot(st_geometry(us_nh_bivalve_mol))
plot(st_geometry(st_centroid(us_nh_bivalve_mol)), add=TRUE)

us_nh_bivalve_pts_latlon <- st_transform(us_nh_bivalve_pts, crs = "+init=epsg:4326") 

mapview(us_nh_bivalve_pts_latlon)

farms_us_nh <- data.frame(do.call(rbind, st_geometry(us_nh_bivalve_pts_latlon))) # get lat lon
names(farms_us_nh) <- c("lon", "lat") 
farms_us_nh <- farms_us_nh %>%
  mutate(iso3c = "USA") %>%
  mutate(data_quality = 4,
...
Map combined with NY, this is what we need (seaweed and shellfish): https://www.arcgis.com/apps/webappviewer/index.html?id=f8799cefbb4c4751a209710d14b9ad46

Current as of 2019

New York and Connecticut’s Shellfish and Seaweed Aquaculture Viewer, 2019. New York Department of Environmental Conservation. https://www.arcgis.com/apps/webappviewer/index.html?id=f8799cefbb4c4751a209710d14b9ad46 
...
Now download using esri2sf R package.

```{r}

url <- "https://services7.arcgis.com/9fAJJI91yoj2y4Yi/arcgis/rest/services/ConnecticutShellfishOnline2018Draft/FeatureServer/1"
ct_shellfish <- esri2sf(url) %>%
  st_as_sf() %>%
  filter(Species != "kelp",
         ActivePerm == "yes")


mapview(ct_shellfish)


us_ct_bivalve_mol <- st_transform(ct_shellfish, crs = "+init=epsg:3035") # convert to equal area projection to find centroids. 
us_ct_bivalve_pts <- st_centroid(us_ct_bivalve_mol)
# check
plot(st_geometry(us_ct_bivalve_mol))
plot(st_geometry(st_centroid(us_ct_bivalve_mol)), add=TRUE)

us_ct_bivalve_pts_latlon <- st_transform(us_ct_bivalve_pts, crs = "+init=epsg:4326") 

mapview(us_ct_bivalve_pts_latlon)

farms_us_ct <- data.frame(do.call(rbind, st_geometry(us_ct_bivalve_pts_latlon))) # get lat lon
names(farms_us_ct) <- c("lon", "lat") 
farms_us_ct <- farms_us_ct %>%
  mutate(iso3c = "USA") %>%
  mutate(data_quality = 4,
...
## Alabama
 ...
 - Here is a map with Shellfish leases!: https://aldcnr.maps.arcgis.com/apps/webappviewer/index.html?id=a32dad8dacd249ea86bcb80dd951a424


Alabama Marine Resources Division: Shellfish Aquaculture Siting Tool, 2021. Alabama Marine Resources Division. https://aldcnr.maps.arcgis.com/apps/webappviewer/index.html?id=a32dad8dacd249ea86bcb80dd951a424
...
Download using esri2sf

```{r}

al_oyster <- esri2sf("https://services7.arcgis.com/jcVUvxpWnajErgCr/arcgis/rest/services/Shellfish_Riparian_Leases_Revised/FeatureServer/0") %>%
  st_as_sf() %>%
  dplyr::filter(Status %in% c("Current", "Current Commercial"))


mapview(al_oyster)

us_al_bivalve_mol <- st_transform(al_oyster, crs = "+init=epsg:3035") # convert to equal area projealion to find centroids. 
us_al_bivalve_pts <- st_centroid(us_al_bivalve_mol)
# check
plot(st_geometry(us_al_bivalve_mol))
plot(st_geometry(st_centroid(us_al_bivalve_mol)), add=TRUE)

us_al_bivalve_pts_latlon <- st_transform(us_al_bivalve_pts, crs = "+init=epsg:4326") 

mapview(us_al_bivalve_pts_latlon)

farms_us_al <- data.frame(do.call(rbind, st_geometry(us_al_bivalve_pts_latlon))) # get lat lon
names(farms_us_al) <- c("lon", "lat") 
farms_us_al <- farms_us_al %>%
  mutate(iso3c = "USA") %>%
  mutate(data_quality = 4,
         subregion = "Alabama") 
```
...
## Washington
 ...
 - This dataset has shellfish farms for Washington and Rhode Island, we will use the Washington farms 
...
dplyr::filter(!(state %in% c("Maine", "Connecticut", "New York", "North Carolina", "Alabama", "New Hampshire", "Alaska", "Rhode Island"))) ## filter out the states we already have
...
mapview(us_aqua_bivalve_pts_latlon)
...
farms_us_aqua <- farms_us_aqua %>%
  mutate(iso3c = "USA", subregion = "Washington") %>%
  mutate(data_quality = 4)
```


## Combine all USA data 

```{r}
farms_usa_total <- rbind(farms_us_aqua, farms_us_ak, farms_us_al, farms_us_ct, farms_us_me, farms_us_nc, farms_us_nh, farms_us_ri)
...
usa_additional <- data.frame(iso3c = "USA", type = "bivalve", num_additional_farms = 193 , data_quality = 4, total_farms = 1203)
...
## UK; Scotland
 - sticking with emodnet for this, since it is comprehensive for the entire country (UK), even if a year behind the scottish gov reporting. 

```{r}
farms_eu_UK <- eu_fao_countries %>%
  filter(COUNTRY == "United Kingdom") # 358.. but only 259 listed as active. Lets compare to the data from scottish gov.  
mapview(farms_eu_UK)

farms_uk <- farms_eu_final %>%
  filter()

## load in data and select active sites, "Seawater" and "Seawater and Freshwater sites", and any sites that contain "Salmon" "Arctic Char" or "Rainbow Trout"

scotland_csv <- as.data.frame(read_csv(file.path(path, "Scotland/Scotland_site_details_2021_raw.csv")))

# Subset to salmonidae farms
sub_scotland <- scotland_csv %>% 
  ...
  filter(`Producing in Last 3 Years` == c("Yes", "New"),
         ...
         str_detect(Species, "mussel|scallop|Oyster|oyster|urchin|Urchin|Scallop|clam|Clam")) # Only 164 reporting actgive here... however, emodnet reports sites not in scotland and in other places in the uk... so i think we should stick with that actually... especially since they grab their data from the same source.. even if outdated. 
...
Source: 
The aquaculture sectors in French Polynesia , 2019. Department of Marine Resources Pu Fa’Ahotu Moana. http://www.ressources-marines.gov.pf/aquaculteurs/laquaculture-en-polynesie-francaise/les-filieres-de-laquaculture-en-polynesie-francaise/
