...
## write out marine fish taxa and production
...
all_groups_included <- aquaculture_groups %>%
  filter(!is.na(value)) %>%
  arrange(-value, country) %>%
  filter(value < 500) %>%  # countries with less do not appear to be stable producers at this point
  group_by(year, iso3c, aq_group) %>%
  summarize(fao_tonnes_production = sum(value)) %>%
  arrange(iso3c, aq_group) %>%
  #filter(year == 2017) %>%
  ungroup()
  #mutate(total_prod = sum(fao_tonnes_production)) %>%
  #mutate(prop_prod = fao_tonnes_production/total_prod)


```

```{r}
## explore out bivalve taxa and production
fao_finfish_taxa_prod <- fao %>%
  filter(species %in% finfish_list) %>%  
  filter(!is.na(value)) %>%
  arrange(-value, country) %>%
...
sum(fao_finfish_taxa_prod$value) # 3916717  

unique(fao_finfish_taxa_prod$country) # 37 countries 
```