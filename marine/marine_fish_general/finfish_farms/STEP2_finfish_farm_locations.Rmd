...
## save farm location
write.csv(farms_all_fin, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/ASC_finfish_farm_lat_lon.csv"), row.names = FALSE)
...
farms_eu_final <- cbind(farms_eu, eu_countries) %>%
 ...
 write_csv(farms_eu_final, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_ESP_CYP_NOR_GRC_MLT.csv"))
...
write_csv(farms_ita, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_ita.csv"))
...
write_csv(farms_fra, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_fra_isr.csv"))
...
write_csv(farms_idn, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_idn.csv"))
...
p_meters <- st_transform(portugal_world_crop, crs = 7801) ## transform to a crs that can buffer in meters 
mapview(p_meters)
...
buffer_outline_fixed <- st_transform(linestring, crs = crs(portugal_world_crop))

linestring_fix <- st_difference(buffer_outline_fixed, portugal) ## now grab only the outline 
mapview(linestring_fix)

portugal_crop <- as(linestring_fix, "Spatial")


portugal_points_lines <- sample.line(portugal_crop, n = 480) ## equally place the number of farms along the coastline 
...
portugal_points_final <- portugal_points[28:139, ] ## grab the appropriate number of points
...
write_csv(farms_prt_final, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_prt.csv"))
```
...
write_csv(farms_irn, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_irn.csv"))
...
write_csv(farms_chn, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_chn.csv"))
...
Source:
National Aquaculture Group, n.d. [Accessed November 12, 2020]. http://www.naqua.com.sa/about-us.php
...
write_csv(farms_sau, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_sau.csv"))
...
write_csv(farms_mus, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_mus.csv"))
...
write_csv(farms_are, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_are.csv"))
...
tunisia_world_crs <- st_transform(tunisia, crs(coast_raw))

tunisia_world_crop <- st_crop(coast_raw, tunisia_world_crs)  %>%
  filter(FID %in% c(652755:652756)) %>%
  mutate(new_id = 1) %>%
  group_by(new_id) %>%
  summarise(geometry = st_union(geometry)) %>%
  ungroup()
...
t_meters <- st_transform(tunisia_world_crop, crs = 7801) ## transform to a crs that can buffer in meters 
mapview(t_meters)

buff <- st_buffer(t_meters, dist = 1000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
...
## now reproject back to original 
buffer_outline_fixed <- st_transform(linestring, crs = crs(tunisia_world_crop))

tunisia2 <- ne_countries(scale = 10, country = "Tunisia", returnclass = "sf")

linestring_fix <- st_difference(buffer_outline_fixed, tunisia2) ## now grab only the outline 
mapview(linestring_fix)

tunisia_crop <- as(linestring_fix, "Spatial")
class(tunisia_crop)
mapview(tunisia_crop)

tunisia_points_lines <- sample.line(tunisia_crop, n = 25) ## equally place the number of farms along the coastline 

tunisia_points <- st_as_sf(tunisia_points_lines) %>%
  mutate(new_id = 1:25) %>%
  filter(new_id %in% c(2:21))
...
## write to farms folder 
write_csv(farms_tun_final, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_tun.csv"))
```
...
## write to farms folder 
write_csv(all_points_twn, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_twn.csv"))
...
Source:
Martec Industries Aquaculture , n.d. . Martec.  https://www.martec.co.cr/#aquaculture
...
write_csv(farms_cri, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_cri.csv"))
...
write_csv(farms_egy, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_egy.csv"))
...
Source:
Indian Council of Agriculture Research - Central Marine Fisheries Research Institute, 2015.  http://www.cmfri.org.in/division/mariculture
...
write_csv(farms_ind, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_ind.csv"))
...
singapore <- ne_countries(scale = 10, country = "Singapore", returnclass = "sf")
...
singapore_world_crs <- st_transform(singapore, crs(coast_raw))

singapore_world_crop <- st_crop(coast_raw, singapore_world_crs) %>%
  dplyr::filter(FID %in% c(242908)) ## filter for the appropriate area

mapview(singapore_world_crop)

s_meters <- st_transform(singapore_world_crop, crs = 7801) ## transform to a crs that can buffer in meters 
mapview(s_meters)

buff <- st_buffer(s_meters, dist = 8000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
...
## now reproject back to original 
buffer_outline_fixed <- st_transform(linestring, crs = crs(singapore_world_crop))


linestring_fix <- st_difference(buffer_outline_fixed, singapore) ## now grab only the outline 
mapview(linestring_fix)



singapore_crop <- as(linestring_fix, "Spatial")
...
## write to farms folder 
write_csv(farms_sgp_final, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_sgp.csv"))
```
...
write_csv(farms_mys, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_mys.csv"))
...
write_csv(farms_bgd, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_bgd.csv"))
...
farms_khm <- data.frame(lat=c(10.53427341558665, 10.748668380457524, 10.647924480438327),
  ...
  mutate(subregion = c("Prey Nob", "Stoeng Hav", "Thomnob Rolork"))

write_csv(farms_khm, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_khm.csv"))
...
south_korea <- ne_countries(scale = 110, country = "South Korea", returnclass = "sf")
...
south_korea_world_crop <- st_crop(world, south_korea_world_crs) # %>%
  # filter(FID %in% c(44825:44924)) %>%
  # mutate(new_id = 1) %>% 
  # group_by(new_id) %>%
  # summarise(geometry %in% st_union(geometry)) %>%
  # ungroup()
 # dplyr::filter(scalerank == 3) ## filter for the appropriate area

mapview(south_korea_world_crop)

k_meters <- st_transform(south_korea_world_crop, crs = 7801) ## transform to a crs that can buffer in meters 
mapview(k_meters)

buff <- st_buffer(k_meters, dist = 20000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
...
## now reproject back to original 
buffer_outline_fixed <- st_transform(linestring, crs = crs(south_korea_world_crop))


linestring_fix <- st_difference(buffer_outline_fixed, south_korea) ## now grab only the outline 
mapview(linestring_fix)

south_korea_crop <- as(linestring_fix, "Spatial")


south_korea_points_lines <- sample.line(south_korea_crop, n = 800) ## equally place the number of farms along the coastline 
...
south_korea_points <- st_as_sf(south_korea_points_lines) %>%
  mutate(new_id = 1:800) %>%
  filter(!(new_id %in% c(96:130, 50:51, 92, 16:37, 712, 753, 665:682, 634:646, 607:620, 561, 531:532, 491, 469:493, 131:183)))
...
## write to farms folder 
write_csv(farms_kor_final, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_kor.csv"))
```
...
# 14.230786241168634, 120.57029349583156
farms_phl <- data.frame(lat=c(14.230786241168634),
                        lon=c(120.57029349583156)) %>%
  mutate(iso3c = "PHL") %>%
  mutate(data_quality = 1) %>%
  mutate(action_needed = "country based suitability")

write_csv(farms_phl, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_phl.csv"))
...
write_csv(farms_hkg, here("marine/marine_fish_general/finfish_farms/data/finfish_sites/farms_hkg.csv"))
...
```{r}
farm_files <- list.files(here::here("marine/marine_fish_general/finfish_farms/data/finfish_sites"), full=TRUE)
