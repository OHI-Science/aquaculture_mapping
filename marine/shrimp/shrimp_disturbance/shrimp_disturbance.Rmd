...
minimum depth of 1 m: assume 1 m depth (no indication of maximum depth, but I don't think we need this anyways, since we have the area already?)
...
1. calculate kg/cage:
   $ area \ m^2 * stocking\  density \frac{kg}{m^2} = kg/cage $
2. Calculate number of cages:
   $ \frac{FAO \ production \ raster \ (kg)}{kg / cage} = cages$

3. calculate surface area disturbance, assume ponds lie next to each other, with no buffer. Cage area will be 0.1 km2:
...
```{r}
production_raster <- rasterFromXYZ(read_csv(paste0(path_prep, "shrimp_farm.csv")), crs = food_crs) 

#plot(production_raster)
cellStats(production_raster, stat = "sum") # 5094133
...
cellStats(shrimp_live_kg, "sum") # 5094133080
```
...
# check
(5094133/t_per_pond)/4 # ~ 955150 if harvest happens 4 times a year

5094133080/kg_per_pond # 3820600
...
cellStats(final_rast, "sum") # 573136.5
final_rast
cellStats(production_raster, "sum") # 5094133
