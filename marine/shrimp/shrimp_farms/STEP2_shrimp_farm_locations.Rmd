...
Current data is from https://zenodo.org/record/3881612 and ASC, only 435 + 4 = 439 farms, so probably not comprehensive, just a drop in the bucket compared to the produciton of China. There are likely many more farms than this. Because we can't find any information on more specific locations or specific number of farms, we will gapfill later based on global tonnes/farm. For now we need to mark the data quality as 2. 
...
farms_chn_2 <- read.csv("/home/shares/food-systems/Food_footprint/_raw_data/Aquaculture_locations/China/split_spp/farms_shrimp_chn.csv") %>%
  dplyr::select(-id) %>%
  mutate(data_quality = 2)

farms_chn_fin <- rbind(farms_chn, farms_chn_2)

write_csv(farms_chn_fin, here("marine/shrimp/shrimp_farms/data/known_farm_sites_2_4_5/farms_chn.csv"))
...
From FAO: http://www.fao.org/fishery/countrysector/naso_madagascar/en
...
write_csv(farms_blz_total, here("marine/shrimp/shrimp_farms/data/known_farm_sites_2_4_5/farms_blz.csv"))
...
Mole, P., Bunge, J., 2002. Shrimp Farming in Brazil: An Industry Overview.
 ...
 - Because this source doesn't have specifics on numbers of farms or where they are located, I will treat this as no information that will need to be fully gapfilled. 
...
Source: 
Van Der Heijden, P., 2020. Aquaculture in Cuba. Aquaculture Europe 45.
