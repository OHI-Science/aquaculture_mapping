...
costa_world_crs <- st_transform(guana_punta, crs(coast_raw))

costa_world_crop <- st_crop(coast_raw, costa_world_crs) %>% ## crop to costa rica extent 
  filter(FID %in% c(245909:245926)) %>% ## filter for the correct coastline 
  mutate(new_id = 1) %>%
  group_by(new_id) %>%
...
c_meters <- st_transform(costa_world_crop, crs = 7801) ## transform to a crs that can buffer in meters 
mapview(c_meters)

buff <- st_buffer(c_meters, dist = 7000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
...
buffer_outline_fixed <- st_transform(linestring, crs = crs(costa_world_crop))
...
costa_crop <- as(linestring_fix, "Spatial")
mapview(costa_crop)

costa_points_lines <- sample.line(costa_crop, n = 106) ## equally place the number of farms along the coastline 

costa_points <- st_as_sf(costa_points_lines) %>%
  mutate(new_id = 1:318) %>%
  filter(new_id %in% c(1:88, 105:106))
mapview(costa_points)


cri_pts <- st_centroid(costa_points)
...
# philly_world_crop <- philly_world_crop %>%
#   mutate(new_id = 1) %>%
#   group_by(new_id) %>%
#   summarise(geometry = st_union(geometry)) %>%
#   ungroup()
...
# 
# ## now reproject back to original
# buffer_outline_fixed <- st_transform(linestring, crs = crs(philly_world_crop))
# mapview(buffer_outline_fixed)
# 
# 
# philly2 <- ne_countries(scale = 50, country = "Philippines", returnclass = "sf")
# mapview(philly2)
# 
# p_meters2 <- st_transform(philly2, crs = 7801) ## transform to a crs that can buffer in meters
# mapview(p_meters2)
# 
# buff2 <- st_buffer(p_meters2, dist = 9000)
# mapview(buff2)
# intersection_fixed <- st_transform(buff2, crs = crs(philly_world_crop))
# mapview(intersection_fixed)
# 
# linestring_fix <- st_difference(buffer_outline_fixed, intersection_fixed) ## now grab only the outline
# mapview(linestring_fix)
...
puttalam_points_lines <- sample.line(puttalam_crop, n = 1361) ## equally place the number of farms along the coastline 
class(puttalam_points_lines)
plot(puttalam_points_lines)

puttalam_points <- st_as_sf(puttalam_points_lines) %>%
  mutate(new_id = 1:1361) %>%
  filter(new_id %in% c(230:1360, 1:194, 199:217))
