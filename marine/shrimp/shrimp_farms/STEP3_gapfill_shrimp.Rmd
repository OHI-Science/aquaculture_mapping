...
farm_files <- list.files(here("marine/shrimp/shrimp_farms/data/known_farm_sites_2_4_5"), full=TRUE)
all_known_farms = plyr::ldply(farm_files, read_csv) %>%
  mutate(type = "shrimp")

number_files <- list.files(here("marine/shrimp/shrimp_farms/data/known_number_farms_3"), full=TRUE)
all_number_3 = plyr::ldply(number_files, read_csv)

number_files_2.5 <- list.files(here("marine/shrimp/shrimp_farms/data/known_subset_2.5"), full=TRUE)
all_number_2.5 = plyr::ldply(number_files_2.5, read_csv)

subset_farms_files <- list.files(here("marine/shrimp/shrimp_farms/data/subset_known_add_points_4"), full=TRUE)
subset_add_4 = plyr::ldply(subset_farms_files, read_csv)

no_info_files <- list.files(here("marine/shrimp/shrimp_farms/data/no_info_1"), full=TRUE)
...
Calculate a global tonnes per farm for shrimp
...
## Join this with the additional farms we need to add to some places, so we can calculate a comprehensive tonnes/farm for shrimp 
...
tonnes_per_farm <- comprehensive_farms_total %>%
  left_join(shrimp_tonnes) %>%
  mutate(tonnes_per_farm = fao_tonnes_production/n_farms)

write.csv(tonnes_per_farm, "output/shrimp_comp_tonnes_per_farm.csv", row.names = FALSE)

avg_global_shrimp_tonnes_per_farm = mean(tonnes_per_farm$tonnes_per_farm)
avg_global_shrimp_tonnes_per_farm # 336.5783

med_global_shrimp_tonnes_per_farm = median(tonnes_per_farm$tonnes_per_farm)
med_global_shrimp_tonnes_per_farm # 174.15
```
...
number_add_points_4 <- subset_add_4 %>%
  ...
  mutate(estimated = 0, 
         avg_tonnes_per_farm = avg_global_shrimp_tonnes_per_farm)
...
write.csv(number_additional_points, here("marine/shrimp/shrimp_farms/output/shrimp_points_to_add_final.csv"), row.names = FALSE)
```
...
all_known_farms_final <- all_known_farms %>%
  dplyr::select(-data_quality, -subregion, -area_ha)


write.csv(all_known_farms_final, here("marine/shrimp/shrimp_farms/output/all_known_shrimp_farms_final.csv"), row.names = FALSE)
...
length(unique(test_all$iso3c)) ## 35 countries


length(unique(shrimp_tonnes$iso3c)) ## 35 countries
