...
## Read in fao data and get a species list for shrimp
...
## write out shrimp taxa and production
...
taxa_group <- fao_shrimp_taxa_prod %>%
  group_by(species) %>%
  summarise(tonnes = sum(fao_tonnes_production)) %>%
  mutate(percent = tonnes/sum(tonnes))
  
write.csv(fao_shrimp_taxa_prod, here("marine/shrimp/shrimp_farms/data/fao_mariculture_shrimp_taxa_production.csv"), row.names=FALSE)
...
write.csv(fao_shrimp, here("marine/shrimp/shrimp_farms/data/fao_mariculture_shrimp.csv"), row.names=FALSE)


## Do some exploration: 

 explore <- read_csv(here("marine/shrimp/shrimp_farms/data/fao_mariculture_shrimp.csv")) %>%
   filter(year==2017) %>%
   arrange(-fao_tonnes_production) %>%
   mutate(percent = round((fao_tonnes_production/sum(fao_tonnes_production)*100),2))

sum(explore$fao_tonnes_production) # 5094133
unique(explore$country) # 35 countries
...
```{r}
## explore out shrimp taxa and production 
fao_shrimp_taxa_prod <- fao %>%
  filter(species %in% shrimp_list) %>%  
  filter(!is.na(value)) %>%
  arrange(-value, country) %>%
...
sum(fao_shrimp_taxa_prod$value) # 5095394 - adds ~1000 tonnes 

unique(fao_shrimp_taxa_prod$country) # 35 countries 
```
