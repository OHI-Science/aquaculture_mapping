...
farms_sf_correct <- farms_sf %>%
  ...
  mutate(GID_0 = ifelse(check %in% c("CHN:HKG", "CHN:MAC"), "CHN", GID_0)) %>%
  mutate(NAME_0 = ifelse(check %in% c("CHN:HKG", "CHN:MAC"), "China", NAME_0)) %>%
  mutate(GID_0 = ifelse(check %in% c("MMR:THA"), "MMR", GID_0)) %>%
  mutate(NAME_0 = ifelse(check %in% c("MMR:THA"), "Myanmar", NAME_0))
...
write_csv(crustacean_raster_df, "/home/shares/food-systems/Food_footprint/all_food_systems/dataprep/aquaculture/marine/crustaceans/crustaceans_farm_updated.csv")
