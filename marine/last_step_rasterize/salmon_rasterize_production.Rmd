...
farms_sf_correct <- farms_sf %>%
  ...
  mutate(GID_0 = ifelse(check %in% "RUS:CHN", "RUS", GID_0)) %>%
  mutate(NAME_0 = ifelse(check %in% "RUS:CHN", "Russia", NAME_0))
...
fao <- read.csv(here("marine/salmon/salmon_farm/data/fao_mariculture_salmon.csv")) %>%
...
write_csv(salmon_raster_df, "/home/shares/food-systems/Food_footprint/all_food_systems/dataprep/aquaculture/marine/salmon/salmon_farm_updated.csv")
