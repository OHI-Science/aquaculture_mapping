...
farms_sf_correct <- farms_sf %>%
  ...
  mutate(GID_0 = ifelse(check %in% "MYS:IDN", "MYS", GID_0)) %>%
  mutate(NAME_0 = ifelse(check %in% "MYS:IDN", "Malaysia", NAME_0)) %>% 
...
st_write(farms_sf_prod, "/home/shares/food-systems/Food_footprint/all_food_systems/dataprep/aquaculture/marine/marine_fish_general/prod_data_by_cell_iso3c.shp", delete_dsn = TRUE)
