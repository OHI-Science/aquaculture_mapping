---
title: "Prepping FAO aquaculture data"
...
This script preps and classifies the FAO aquaculture data to the appropriate production types and species. 
...
source(here('R/fao_mar_clean.R')) # functions specific to mariculture dealing with compound countries
...
fao <- fao %>%
  ...
  dplyr::group_by(country, species, fao_major_area, environment) %>%
  dplyr::mutate(total_harvest = sum(value, na.rm=TRUE)) %>%
  dplyr::filter(total_harvest > 0) %>% # data is cut if country/species/major area hasn't been harvested since 2010
...
table(fao$iso3c)
## save
write.csv(fao, here("data/fao_aquaculture_clean.csv"), row.names=FALSE)
...
write.csv(fw, "marine/STEP1_species_groups/int/fw_spp.csv", row.names = FALSE)
...
## Prep marine and brackish-water
```{r}
...
fao_marine_fw_not_included <- fao_marine %>%
  ...
  dplyr::filter(str_detect(species, "Freshwater|catfish|Cyprinids|ilapia|iver")) ## look at those species we will be cutting (because they are freshwater)
...
fao_marine_clean <- fao_marine %>%
  ...
  dplyr::filter(!str_detect(species, "Freshwater|catfish|Cyprinids|ilapia|iver"),
                Taxon_code != "AL",
                species != "Red oyas",
                species != "Jellyfishes nei") ## delete freshwater, catfish species, tilapia, river species, seaweeds, jellyfish..


```

Classify species into mariculture groups consistent with MacLeod et al. 2020 (https://www.nature.com/articles/s41598-020-68231-8)

Need to classify into these groups, for easier mapping and pressure calculations:
 - Bivalves (Unfed or algae fed bivalve molluscs)
 - Tuna (Bluefin tuna)
 - Salmonids (Salmonidae fish)
 - Shrimps and Prawns
 - crustaceans (Non-shrimp crustaceans)
 - marine fish general (General marine fish)
...
## CRUST - classified as crustaceans
...
## write it out
write.csv(aquaculture_groups, "marine/STEP1_species_groups/int/marine_groups.csv", row.names = FALSE)


aq_groups <- read.csv("marine/STEP1_species_groups/int/marine_groups.csv")


fao_1 <- read.csv(here("data/fao_aquaculture_clean.csv")) %>%
  filter(environment %in% c("Marine", "Brackishwater")) %>%
  left_join(aq_groups) %>%
...
write.csv(fao_1, "marine/STEP1_species_groups/int/tonnes_per_country_group.csv", row.names = FALSE)
...
test <- fao_1 %>%
  dplyr::filter(aq_group == "salmonids")
sum(test$tonnes) # 2753752
...
fao_1 <- read.csv(here("data/fao_aquaculture_clean.csv")) %>%
 ...
 # arrange(aq_group)
sum(fao_1$total_tonnes) # 29662247 reported 
...
sum(fao_1$total_tonnes) # 38731.97

38731.97/29662247 # we exclude around 0.1% of marine aquaculture



## what is the proportion of marine salmon aquaculture that we include?

# we include 2753752 tonnes of marien salmon aquaculture
fao_1 <- read.csv(here("data/fao_aquaculture_clean.csv")) %>%
 filter(environment %in% c("Marine", "Brackishwater")) %>%
  left_join(aq_groups) %>%
  dplyr::filter(year == 2017,
...
sum(fao_1$total_tonnes) # 2754748

2753752/2754748 # we include 0.9996384


## What proportion does freshwater salmon production account for? 
fao_1 <- read.csv(here("data/fao_aquaculture_clean.csv")) %>%
 # filter(environment %in% c("Marine", "Brackishwater")) %>%
  left_join(aq_groups) %>%
  dplyr::filter(year == 2017, 
                Taxon_code %in% c(NA, "F"),
                aq_group %in% c(NA, "salmonids")) %>%
  dplyr::filter(aq_group == "salmonids"|str_detect(species, "trout|Trout|salmon|Salmon|char|Char|whitefish")) %>%
  # dplyr::filter(year == 2017,
               # !is.na(value)) %>%
                #!is.na(aq_group)) %>%
  # dplyr::filter(value < 500) %>%
  # dplyr::filter(aq_group == "salmonids") %>%
  group_by(aq_group, country, species, year, environment) %>%
  summarise(tonnes = sum(value)) %>%
  ungroup()  %>%
  # dplyr::filter(tonnes >= 500) %>%
...
# A tibble: 3 × 2
#   environment   total_tonnes
#   <chr>                <dbl>
# 1 Brackishwater       28086.
# 2 Freshwater         676537.
# 3 Marine            2726662.

2726662 + 28086 # 2754748


676537/2754748 # 0.2455894
```
