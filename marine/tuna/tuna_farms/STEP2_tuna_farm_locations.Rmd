...
fao_tuna_spp <- read_csv("marine/tuna/tuna_farms/data/fao_mariculture_tuna_taxa_production.csv") %>%
  dplyr::filter(year == 2017)
fao_tuna <- read_csv("marine/tuna/tuna_farms/data/fao_mariculture_tuna.csv") %>%
  dplyr::filter(year == 2017)
...
Source:
EMODnet Human Activities: Marine Finfish Aquaculture, 2019. [Accessed December 8, 2020] 
https://www.emodnet-humanactivities.eu/search-results.php?dataname=Finfish+Production
...
farms_eu_final <- cbind(farms_eu, eu_countries) %>%
 ...
 write_csv(farms_eu_final, here("marine/tuna/tuna_farms/data/known_farm_sites_2_4_5/farms_ESP_MLT.csv"))
...
Croatia: 
Need to add 14 points to existing 1 (a final total of 15 points)

Current data is from NASO maps collection and is only a subset of farms 

NASO aquaculture maps collection , 2010. . Food and Agriculture Organization of the United Nations. [Accessed January 17, 2020] http://www.fao.org/fishery/naso-maps/selected-aquaculture-sites/

We need to add 14 farms to match the total listed by this government source: 
...
Turkey:
Need to add 5 points to existing 3 (a final total of 8 points)

Data is from NASO maps collection, so only a subset

However the FAO gives us specific numbers 
...
med_countries <- med_aqua %>%
  dplyr::mutate(country = sub("_.*", "", Name)) %>%
  dplyr::mutate(country = sub(" fishpen A", "", country))

unique(med_countries$country)

## We need to filter for just the appropriate countries (that we don't already have): Croatia, Turkey
...
write_csv(farms_med, here("marine/tuna/tuna_farms/data/known_farm_sites_2_4_5/farms_hrv_tur.csv"))
...
hrv_additional <- data.frame(iso3c = "HRV", type = "tuna", num_additional_farms = 14, data_quality = 4, total_farms = 15)
write.csv(hrv_additional, here("marine/tuna/tuna_farms/data/subset_known_add_points_4/add_farms_hrv.csv"), row.names = FALSE)

#### Turkey 
tur_additional <- data.frame(iso3c = "TUR", type = "tuna", num_additional_farms = 5, data_quality = 4, total_farms = 8)
write.csv(tur_additional, here("marine/tuna/tuna_farms/data/subset_known_add_points_4/add_farms_tur.csv"), row.names = FALSE)
...
Source:  
Aquaculture public register - Government of South Australia, 2017. [Accessed November 14, 2020] https://www.pir.sa.gov.au/aquaculture/aquaculture_public_register

```{r}
south_au <- read_csv(file.path("/home/shares/food-systems/Food_footprint/_raw_data/Aquaculture_locations/Australia/tuna/south_au_leases.csv")) 
...
for(i in 2:seq_along(tuna_links)){
...
write_csv(lat_lon_final, here("marine/tuna/tuna_farms/data/known_farm_sites_2_4_5/farms_AUS.csv"))
```
...
Source:
FAO Fisheries & Aquaculture - National Aquaculture Sector Overview - Japan, 2017. National Aquaculture Sector Overview Fact Sheets.

```{r}
farms_jpn <- data.frame(iso3c = "JPN", type = "tuna", num_additional_farms = 92, data_quality = 3)

write_csv(farms_jpn, here("marine/tuna/tuna_farms/data/known_number_farms_3/farms_jpn.csv"))
```
...
Source:
Monterey Bay Aquarium Seafood Watch Report on Pacific Bluefin Tuna Farming in Mexico, 2016.
...
write_csv(farms_mex, here("marine/tuna/tuna_farms/data/known_farm_sites_2_4_5/farms_MEX.csv"))
...
```{r}
farm_files <- list.files(here::here("marine/tuna/tuna_farms/data/known_farm_sites_2_4_5"), full=TRUE)
...
write.csv(all_farms_1, here::here("marine/tuna/tuna_farms/data/global_tuna_farm_lat_lon.csv"), row.names = FALSE)

write.csv(all_farms, here::here("marine/tuna/tuna_farms/data/global_tuna_farm_lat_lon_data_quality.csv"), row.names = FALSE)
