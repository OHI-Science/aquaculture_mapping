...
Now we will map the remaining countries with tuna farms that are not included in the ASC dataset. Workflow will be as follows: 
...
farms_eu_final <- cbind(farms_eu, eu_countries) %>%
 ...
 write_csv(farms_eu_final, here("marine/tuna/tuna_farms/data/tuna_sites/farms_ESP_MLT.csv"))
...
write_csv(farms_med, here("marine/tuna/tuna_farms/data/tuna_sites/farms_hrv_tur.csv"))
...
write_csv(lat_lon_final, here("marine/tuna/tuna_farms/data/tuna_sites/farms_AUS.csv"))
```
...
j_meters <- st_transform(japan_world_crop, crs = 7801) ## transform to a crs that can buffer in meters 
mapview(j_meters)

buff <- st_buffer(j_meters, dist = 7000) # buffer out to XX km so that we can place points along a line 1 km from the coastline
mapview(buff)
...
buffer_outline_fixed <- st_transform(linestring, crs = crs(japan_world_crop))

japan <- ne_countries(scale = 10, country = "Japan", returnclass = "sf")

linestring_fix <- st_difference(buffer_outline_fixed, japan) ## now grab only the outline 
mapview(linestring_fix)


japan_crop <- as(buffer_outline_fixed, "Spatial")


japan_points_lines <- sample.line(japan_crop, n = 190) ## equally place the number of farms along the coastline 
...
japan_points <- st_as_sf(japan_points_lines) %>%
  mutate(new_id = 1:190) %>%
  filter(!(new_id %in% c(2, 95, 73, 67, 62, 56, 48, 47, 33,  32, 31, 29, 23, 12, 9,  93:112, 116, 118, 121, 123:126, 129:140, 124:145, 148:167, 171:174, 176, 178:190)))
...
write_csv(farms_jpn_final, here("marine/tuna/tuna_farms/data/tuna_sites/farms_jpn.csv"))
```
...
write_csv(farms_mex, here("marine/tuna/tuna_farms/data/tuna_sites/farms_MEX.csv"))
...
```{r}
farm_files <- list.files(here::here("marine/tuna/tuna_farms/data/tuna_sites"), full=TRUE)
