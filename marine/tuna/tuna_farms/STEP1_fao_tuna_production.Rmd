...
## write out tuna taxa and production
...
write.csv(fao_tuna_taxa_prod, here("marine/tuna/tuna_farms/data/fao_mariculture_tuna_taxa_production.csv"), row.names=FALSE)
...
write.csv(fao_tuna, here("marine/tuna/tuna_farms/data/fao_mariculture_tuna.csv"), row.names=FALSE)
        
 explore <- read_csv(here("marine/tuna/tuna_farms/data/fao_mariculture_tuna.csv")) %>%
   filter(year==2017) %>%
   arrange(-fao_tonnes_production) %>%
   mutate(percent = round((fao_tonnes_production/sum(fao_tonnes_production))*100, 2))
 sum(explore$fao_tonnes_production) # 37072.56
 unique(explore$country) # 7 countries 
```
...
sum(fao_tuna_taxa_prod$value) # 37072.56

unique(fao_tuna_taxa_prod$country) # 16 countries 
```
