...
```{r}
occupancy <- read_csv(file.path(path_prep, "salmon_disturbance.csv"))

water <- occupancy %>% 
  mutate(salmon_water_m3 = ifelse(salmon_disturbance_prop == 0, NA,
                                  ifelse(salmon_disturbance_prop > 0, 0, salmon_disturbance_prop))) %>% 
