...
fao_salmon <- read_csv("marine/salmon/salmon_farms/data/fao_mariculture_salmon.csv") %>%
...
sub_scotland <- scotland_csv %>% 
  ...
  filter(`Producing in Last 3 Years` %in% c("Yes", "New"),
         `Water Type` %in% c("Seawater", "Freshwater and Seawater"),
         str_detect(Species, "Salmon|Trout"))

unique(sub_scotland$Species)



## While there are >350 sites in the data which are deemed to have produced Salmonidae species, only 110 of them have operated in the past 3 years, which is what we include in our map. 
...
write_csv(farms_scotland, here("marine/salmon/salmon_farms/data/known_farm_sites_2_4_5/farms_scotland.csv"))
...
Locations of aquaculture leases in Maine, 2019. Maine Department of Marine Resources. 
 - Viewer: https://maine.maps.arcgis.com/apps/webappviewer/index.html?id=b846cf37b1d64c988f89eafa085c8b7a
 - Download: https://dmr-maine.opendata.arcgis.com/datasets/maine::mainedmr-aquaculture-aq-leases/about
 - We will want to use regular leases only.
 ...
 - Update to date as of download date
 - Has species types specified. 
 
 - geojson API link for download: https://opendata.arcgis.com/datasets/a0b3c775cfc243a2b92df328ad85c642_2.geojson


```{r}
me_geojson <- geojson_read("https://opendata.arcgis.com/datasets/a0b3c775cfc243a2b92df328ad85c642_2.geojson",  what = "sp") %>% st_as_sf() %>%
  mutate(download_date = date, 
         data_year = year) %>%
  clean_names()
mapview(me_geojson)

#st_write(me_geojson, "/home/shares/food-systems/Food_footprint/_raw_data/Aquaculture_locations/USA/Maine/MaineDMR_Aquaculture_AQ_Leases/MaineDMR_Aquaculture_AQ_Leases.shp", delete_dsn = TRUE)
...
maine_aq_lease_active <- maine_aq_lease %>%
  ...
  dplyr::filter(str_detect(PRIMARYSP, "Salmon")) # 22 leases.. matches the northeast data
 
us_me_salmon_mol <- st_transform(maine_aq_lease_active, crs = "+init=epsg:3035") # convert to equal area projection to find centroids.
us_me_salmon_pts <- st_centroid(us_me_salmon_mol)
# check
plot(st_geometry(us_me_salmon_mol))
plot(st_geometry(st_centroid(us_me_salmon_mol)), add=TRUE)

us_me_salmon_pts_latlon <- st_transform(us_me_salmon_pts, crs = "+init=epsg:4326")

farms_us_me <- data.frame(do.call(rbind, st_geometry(us_me_salmon_pts_latlon)))
names(farms_us_me) <- c("lon", "lat")
farms_us_me <- farms_us_me %>%
  mutate(iso3c = "USA") %>%
  mutate(data_quality = 2) %>%
  mutate(subregion = "Maine")

write_csv(farms_us_me, here("marine/salmon/salmon_farms/data/known_farm_sites_2_4_5/farms_maine_usa.csv"))
...
farms_ps <- read.csv(file.path(path, "USA/PugetSound/USA_PugetSound_AtSalmon.csv")) %>%
  ...
  dplyr::select(lon, lat, iso3c) %>%
  mutate(data_quality = 2) %>%
  mutate(subregion = "Washington")

write_csv(farms_ps, here("marine/salmon/salmon_farms/data/known_farm_sites_2_4_5/farms_ps_usa.csv"))
...
farm_files <- list.files(here("marine/salmon/salmon_farms/data/known_farm_sites_2_4_5"), full=TRUE)
all_farms = plyr::ldply(farm_files, read_csv)

all_farms_1 <- all_farms %>%
  dplyr::select(-data_quality)

write.csv(all_farms_1, here("marine/salmon/salmon_farms/data/global_salmon_farm_lat_lon.csv"), row.names = FALSE)

write.csv(all_farms, here("marine/salmon/salmon_farms/data/global_salmon_farm_lat_lon_quality.csv"), row.names = FALSE)
