...
## 16 countries/species group combos that will need spatial allocation according to our rules (7-10km to port, depth requirements)


n_countries_no_data <- n_countries %>%
  dplyr::filter(data_quality == 1)
sum(n_countries_no_data$n_countries) # 14
## 14 countries/species group combos that will need country level spatial allocation according to our rules (7-10km to port, depth requirements)

n_states_no_data <- n_countries %>%
  dplyr::filter(data_quality == 2)
sum(n_states_no_data$n_countries) # 14
