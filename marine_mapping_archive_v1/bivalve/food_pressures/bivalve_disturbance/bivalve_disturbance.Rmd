...
We assuming that the rearing time is about 1 year, so no lost bivalves. What is reported is the actual tonnage. 
...
We will also use information for oyster species from here (Ostreidae): **be sure to include this as a reference in the supplementary info**
...
write.csv(bivalve_w_l_final, file.path("marine/bivalve/food_pressures/bivalve_disturbance/data/weight_length_relationships.csv"), row.names = FALSE)
...
bivalve_spp <- read_csv(file.path("marine/bivalve/bivalve_farms/data/fao_mariculture_bivalve_taxa_production.csv"))


## read in family information
spp_families <- read_csv(file.path("marine/STEP1_species_groups/int/marine_groups.csv")) %>%
...
# 1. stack with the region raster and covert to data.frame.  merge with the dataframe that will convert the ID_0 values to iso3c. 
...
production <- read_csv(file.path(path_prep, "bivalve_farm.csv"))
sum(production$tonnes_production)

iso3c_codes <- read_csv(file.path("_spatial/_output/eez_food_rgns.csv"))

land_eez_region_raster <- raster("/home/shares/food-systems/Food_footprint/dataprep/spatial/land_eez_rgns.tif")
