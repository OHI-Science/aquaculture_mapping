...
write_csv(farms_idn, here("marine/crustaceans/crustacean_farms/data/crustacean_sites/farms_idn.csv"))
...
write_csv(farms_chn, here("marine/crustaceans/crustacean_farms/data/crustacean_sites/farms_chn.csv"))
...
farms_bgd <- data.frame(lat = c(21.876081791221054, 22.325670145257305),
                        lon = c(89.49499697244659, 91.76091935028879)) %>%
  mutate(iso3c = "BGD") %>%
  mutate(data_quality = 2) %>%
  mutate(action_needed = "state/province based suitability",
         subregion = c("Khulna", "Chittagong"))

write_csv(farms_bgd, here("marine/crustaceans/crustacean_farms/data/crustacean_sites/farms_bgd.csv"))
...
farms_mmr <- data.frame(lat = c(15.930407995930784, 18.615093163569437),
  ...
  mutate(data_quality = 2) %>%
  mutate(action_needed = "state/province based suitability",
         subregion = c("Ayeyarwaddy", "Rakhine"))

write_csv(farms_mmr, here("marine/crustaceans/crustacean_farms/data/crustacean_sites/farms_mmr.csv"))
...
farms_vnm <- data.frame(lat = c(21.186050529545593, 20.840947248646557, 20.454148639095266, 20.146554460540862, 19.955496263213185, 19.726817666482322, 18.982876968029743, 18.456817613512555, 17.56001328746647, 16.905826522213328, 16.495115996149718, 16.167986126005765, 15.699026568754403, 15.010595152983287, 14.17668770026659, 13.335760529041393, 12.299111935360338, 11.62183927318579, 10.830102342204183, 10.510731358280973, 10.30006571666483, 10.037866832581582, 9.592223563185232, 9.389580381208823, 9.147921315425062, 8.674166818318303, 10.087872204898957, 10.055819944690162),
                        lon = c(107.60858937642442, 106.77685003845498, 106.59728856581118, 106.3298019452741, 106.03097468374126, 105.87661730467457, 105.6165097440173, 105.92726656633255, 106.58039999954293, 107.19194145855845, 107.74891583338649, 108.15075180169012, 108.48016289264334, 108.92174225624275, 109.19617116389627, 109.29183511586322, 109.23699322130719, 109.1546891359726, 108.03702110643216, 106.7310999868549, 106.78122716883196, 106.69637036338817, 106.54126727713718, 106.1857446280396, 105.59613926925006, 105.12379673906345,  104.88743812284027, 105.79073953273914),
                        prod = c(0, 1650, 0, 0, 1005, 970, 0, 200, 110, 20, 82, 0, 135, 0, 130, 0, 0, 5, 0, 250, 0, 637, 1196, 0, 3466, 0, 170, 0), subregion = c("Quang Ninh", "Hai Phong", "Thai Binh", "Nam Dinh", "Ninh Binh", "Thanh Hoa", "Nghe An", "Ha Tinh", "Quang Binh", "Quang Tri", "TT Hue", "Da Nang", "Quang Nam", "Quang Ngai", "Binh Dinh", "Phu Yen", "Khanh Hoa", "Ninh Thuân", "Binh Thuân", "Long An", "Tien Giang", "Ben Tre", "Tra Vinh", "Soc Trang", "Bac Lieu", "Ca Mau", "Kien Giang", "Can Tho")) %>%
  mutate(iso3c = "VNM") %>%
  mutate(data_quality = 2) %>%
  mutate(action_needed = "state/province based suitability")

write_csv(farms_vnm, here("marine/crustaceans/crustacean_farms/data/crustacean_sites/farms_vnm.csv"))
...
phl_lat_lon <- phl_mudcrab_prod %>%
  ...
  dplyr::select(lat, lon, prod, "subregion" = "Province") %>%
  mutate(iso3c = "PHL") %>%
  mutate(data_quality = 2) %>%
  mutate(action_needed = "state/province based suitability")


write_csv(phl_lat_lon, here("marine/crustaceans/crustacean_farms/data/crustacean_sites/farms_phl.csv"))
...
```{r}
farm_files <- list.files(here::here("marine/crustaceans/crustacean_farms/data/crustacean_sites"), full=TRUE)
...
write.csv(all_farms, here::here("marine/crustaceans/crustacean_farms/data/global_crustacean_farm_lat_lon_data_quality.csv"), row.names = FALSE)

all_farms_1 <- all_farms %>%
  dplyr::select(-data_quality, -action_needed)

write.csv(all_farms_1, here::here("marine/crustaceans/crustacean_farms/data/global_crustacean_farm_lat_lon.csv"), row.names = FALSE)
```
