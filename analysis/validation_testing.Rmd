...
```{r}
validation_all <- readRDS(file.path("data/all_validation_farms_fix_shrimp.rds"))  %>%
...
sum(eez_cells_df$predicted, na.rm = TRUE) # 8442
...
Use our EEZ mask to create a confusion matrix for CEA 25km2 cell sizes 
 ...
 - AUC of ~0.58 - worse than 36km2... makes sense! 
```{r}
...
sum(eez_cells_df$predicted, na.rm = TRUE) # 8445
...
Use our EEZ mask to create a confusion matrix for 0.25 X 0.25 degree cell sizes (about 769.3152km2 at equator). Average cell size is 490.3108 km2
 ...
 - AUC of ~7042 ; makes sense that is it better! 
...
## still missing some farms.. but nothing egregious. This is probably ok. The results wouldn't change much even if we had the extra farms. 
