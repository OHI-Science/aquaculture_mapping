...
all_farms <- readRDS("data/all_modeled_marine_farms_31km.rds") %>%
...
mutate(species_group = case_when(species == "Salmonids" ~ "Salmonidae fish",
  ...
  mutate(data_type_2 = ifelse(str_detect(details, "Although"), "E", data_type_2)) %>%
  mutate(data_type_2 = ifelse(iso3c == "IND" & species == "Unfed or algae fed shellfish", "E", data_type_2)) ## fix some mismatches


## data types with comprehensive # of farms are A, B, and C. These all recieve their production/# of farms to get prod per farm. 

## data types D, and E recieve global prod per farm estimates. 
...
st_write(all_farms_sources_final, "all_marine_aquaculture_farms_sources_31km.csv", layer_options = "GEOMETRY=AS_XY", delete_dsn = TRUE)
