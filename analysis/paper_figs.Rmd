...
all_farms <- read_csv("all_marine_aquaculture_farms_sources_31km.csv") %>% 
  mutate(data_type_plot = case_when(
    data_type == "modeled location, known number of farms" ~ "Modeled location, known number of farms",
    data_type == "modeled location, estimated number of farms" ~ "Modeled location, estimated number of farms", 
    data_type == "known location" ~ "Known location"
  ))
...
ggdraw(plot_final)
ggsave("analysis/figs/map_fig_18km.jpg")
...
## Plot with norway instead of USA
...
## we saved both of those images separately. Then we export them to google drawings, and combine together. Once combined, reupload to figs folder as "zoom_fig_final_nor.jpg"
...
ARCHIVE: Radial bar chart of estimation type per country
