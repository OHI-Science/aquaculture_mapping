...
## How much mariculture production was included in our assessment? 
...
## total marine animal production is: 8090542 + 22920871 = 31011413
...
all_comp_farms <- rbind(finfish_comp, bivalve_comp, salmon_comp, tuna_comp, shrimp_comp) %>%
    mutate(Species = case_when(
      type == "salmon" ~ "Salmonids", 
      type == "bivalve" ~ "Unfed or algae fed shellfish",
...
comp_across_all <- all_comp_farms %>%
  full_join(country_spp_combos, by = c("Country" = "country", "Species" = "species")) %>%
...
sum(comp_across_all$tonnes) # account for 4328484 tonnes of production 

4328484/29623515 # these account for ~15% of production tonnage

## these are the countries that are comprehensive across all species groups they report
cat(comp_across_all$Country, sep = ", ")

# tuna_spp <- read_csv("marine/tuna/tuna_farms/data/fao_mariculture_tuna_taxa_production.csv")
```
