...
## ok so that works... i just to calculate the distance from each point in the whole known location dataset, and find the min of each column
...
mean(spp_group_dist$closest_port_km) # 31.30463 km
sd(spp_group_dist$closest_port_km) # 32.333 km
range(spp_group_dist$closest_port_km) #  0.1312871 to 531.2067646
median(spp_group_dist$closest_port_km) # 18.85009 km
...
spp_group_dist_final <- spp_group_dist %>%
  group_by(type) %>%
  summarise(dist_port = mean(closest_port)) %>%
  ungroup()
