...
aq_info <- read_csv('/home/shares/clean-seafood/raw_data/FAO/production/CL_FI_COUNTRY_GROUPS.csv') %>% 
  clean_names() %>% 
...
aq_types <- raw %>% 
  ...
  group_by(continent_group, group) %>% 
  summarise(tonnes_per_farm = mean(tonnes_per_farm))
```
