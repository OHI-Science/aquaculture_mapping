...
Note: We will use the same pond dimensions (other than stocking density), as crustacean. The paper states that mud crab earthen ponds are the same as shrimp farms, and sometimes even switch between crab and shrimp farmings. 
...
```{r}
production_raster <- rasterFromXYZ(read_csv(paste0(path_prep, "crustaceans_farm.csv")), crs = food_crs) 

#plot(production_raster)
cellStats(production_raster, stat = "sum") # 401719.1
...
cellStats(crustacean_live_kg, "sum") # 534286390
cellStats(crustacean_live_tonnes, "sum") # 534286.4

401719.1*0.33 # 132567.3

132567.3 + 401719.1 # 534286.4
```
...
# check
(534286.4/t_per_pond)/4 # ~ 2145.729 ponds, if harvest happens 4 times a year
...
cellStats(final_rast, "sum") # 1294.5
final_rast
cellStats(production_raster, "sum") # 401719.1
